...
public class AlipayNotify {
     ...
     * @return 验证结果
     */
    public static boolean verify(Map<String, String> params, AlipayConfig config, boolean checkNotifyId) {

        //判断responsetTxt是否为true，isSign是否为true
        //responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
        //isSign不是true，与安全校验码、请求时的参数格式（如：带自定义参数等）、编码格式有关
        String responseTxt = "true";
        if (checkNotifyId && params.get("notify_id") != null) {
     ...
     * @return 生成的签名结果
     */
    private static boolean getSignVeryfy(Map<String, String> Params, String sign, AlipayConfig config) {
        ...
        if (AlipayConfig.SIGN_TYPE.equals("MD5")) {
            isSign = AlipayMD5.verify(preSignStr, sign, config.getKey(), AlipayConfig.INPUT_CHARSET);
        }
        return isSign;
    }
